...
BUDGET_TOKENS = int(sys.argv[2]) if len(sys.argv) > 2 else 24_000
...
def resumer(messages_a_resumer):
    """Demande au modèle un résumé de l'historique."""
    texte = "\n".join(
...
if len(sys.argv) < 2:
    print('Usage : uv run python exemples-code/04-gestion-contexte.py "votre question" [budget_tokens]')
    sys.exit(1)
